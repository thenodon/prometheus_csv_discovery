...
.PHONY: build
build:
	CGO_ENABLED=0 go build ${LDFLAGS} -v -o target/prometheus-cvs-discovery .
...
build-release-amd64:
	CGO_ENABLED=0 GOOS=linux   GOARCH=amd64 go build ${LDFLAGS} -o=prometheus-cvs-discovery.linux.amd64 .        && \
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build ${LDFLAGS} -o=prometheus-cvs-discovery.windows.amd64.exe .  && \
	CGO_ENABLED=0 GOOS=darwin  GOARCH=amd64 go build ${LDFLAGS} -o=prometheus-cvs-discovery.darwin.amd64 .

.PHONY: build-release-arm64
build-release-arm64:
	CGO_ENABLED=0 GOOS=linux   GOARCH=arm64 go build ${LDFLAGS} -o=prometheus-cvs-discovery.linux.arm64 .        && \
 	CGO_ENABLED=0 GOOS=darwin  GOARCH=arm64 go build ${LDFLAGS} -o=prometheus-cvs-discovery.darwin.arm64 .
